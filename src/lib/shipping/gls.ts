...
/* GLS MyGLS API (JSON). Docs: https://api.mygls.cz/docs/ — username + password + client number from the GLS account. */
const base = "https://api.mygls.cz/ParcelService.svc/json";

export async function glsShipment(order: Order): Promise<Shipment> {
    const username = process.env.GLS_USERNAME; const password = process.env.GLS_PASSWORD; const clientNumber = process.env.GLS_CLIENT_NUMBER;
